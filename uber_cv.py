...
#_0000_list_100r_96T_50reg.fit0__0000_quality_100%.fit
image_file = "C:/Users/Vito/Downloads/0__0001_quality_100%.fit"
...
def intenziteta(cx,cy, r_max,n_bins=200):
    ...
    #r_max = 1320 #Radij v px
    dr = r_max / n_bins #d kolobar
    #Rezultati
    radij = []
    svetlost = []
    nedolocenost = []
    for r_ in np.linspace(0,r_max, n_bins): #Kolobarji
...
def md_2(radij, svetlost):
    ...
    # --- CELI INTERVAL ---
    # Za izračun RMSE vzamemo vse točke, kjer je koren definiran (varnostna rezerva 0.5)
    maska_celi = x < (R_konstanta - 0.5)
    x_celi = x[maska_celi]
    y_celi = y[maska_celi]

    # --- POLOVICA ZA FITANJE ---
    # Algoritem prilagaja parametre le na podlagi prve polovice podatkov
    polovica_radija = np.max(x) * (100/100)
    maska_polovica = x < polovica_radija
    x_fit = x[maska_polovica]
    y_fit = y[maska_polovica]

    # 2. DEFINICIJA MODELA (iz prve slike)
    def funck(r, tau):
        R = 1324 
        return np.exp(-tau * (1 / (np.sqrt(1 - (r / R)**2)) - 1))

    # 3. IZVEDBA FITANJA (na prvi polovici)
    try:
        ...
        # --- IZRAČUN RMSE NA POLOVICI INTERVALA (kjer se je fitalo) ---
        ...
        # --- IZRAČUN RMSE NA CELEM INTERVALU ---
    ...
    # Osvatljeno območje, ki je služilo za fitanje
    #plt.axvspan(0, polovica_radija, color='blue', alpha=0.1, label='Uporabljeno za fit')

    if tau_opt is not None:
        # Izris optimalne krivulje čez celoten definiran interval
...
def md3(radij, svetlost):
    ...
    # 2. DEFINICIJA MODELA (iz slike image_cd5f0f.png)
    ...
    # 3. IZVEDBA FITANJA (na prvi polovici)
    # p0=[0.3, 1.5] so začetni približki za [tau, t_ratio]
    try:
...
def mdu(radij, svetlost):
    ...
    # Če fitamo R, je R zadnji parameter, sicer je fiksen
        R = radij
        u = params[0] # prvi parameter je u
    ...
    # 2. IZVEDBA FITANJA
    # p0: [u, (opcijsko R)]
    p0 = [0.6] # začetni približek za u je tipično okoli 0.6
...
def md4(radij, svetlost, n=2, show= False,R_konstanta=1320):
    ...
    fit_R = False  # STIKALO: True = R se fit-a, False = R ostane fiksen
    ...
    # 2. DEFINICIJA MODELA
    # Če fit_R=True, funkcija sprejme R kot zadnji parameter
    def funk_model4(r, *params):
        if fit_R:
            # Zadnji parameter je R, ostali so koeficienti 'a'
            a = params[:-1]
            R = params[-1]
        else:
            # Vsi parametri so koeficienti 'a', R je fiksen
            a = params
            R = R_konstanta
            
        cos_theta = np.sqrt(np.maximum(0, 1 - (r / R) ** 2)) # maximum prepreči koren neg. št.
        I = 0
    ...
    # 3. IZVEDBA FITANJA
    # Začetni približki: [a0, a1, a2, (opcijsko R)]
    # iteracije
    p0 = n*[1]
    if fit_R:
    ...
    if show:
        ...
        if popt is not None:
            # 1. Priprava besedila za koeficiente
            # Izpišemo koeficiente a0, a1, a2, ...
            tekst_koef = "\n".join([f"a{i} = {val:.3f}" for i, val in enumerate(popt[:-1] if fit_R else popt)])
            #tekst_koef += f"\nR = {popt[-1]:.1f}"
            # 2. Dodajanje anotacije na graf
            plt.annotate(f'Koeficienti:\n{tekst_koef}', 
                        xy=(0.02, 0.05), # Postavi v spodnji levi kot (axes fraction)
...
cx,cy,rad = doloci_soncev_radij(image_file)
radij, svetlost, nedolocenost = intenziteta(cx,cy,rad)
svetlost = svetlost/svetlost[0]
#u = mdu(radij, svetlost)
popt = md4(radij, svetlost, n=8, R_konstanta=rad)
tau, T = tempstvar(popt)
plt.plot(tau, T, color='red')
plt.show()
#plt.plot(radij, svetlost, color='red')
#plt.show()
#print(cx ,cy, rad)